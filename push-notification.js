// ============================================================
// push-notification.js - مدیریت Notification + Archive
// ============================================================
...
class PushNotificationManager {
    constructor() {
        ...
        this.WORKER_URL = 'https://your-worker-name.workers.dev';
    ...
    async saveNotification(notification) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const data = { ...notification, timestamp: notification.timestamp || Date.now(), isRead: notification.isRead || false,
                isSynced: false };
    ...
    async getNotifications() {
        ...
        return new Promise((resolve, reject) => {
            ...
            request.onsuccess = (event) => {
                ...
                if (cursor) { results.push(cursor.value);
    ...
    async saveSubscription(subscription) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const data = { endpoint: subscription.endpoint, keys: subscription.keys, createdAt: Date.now() };
            const request = store.add(data);
            request.onsuccess = () => resolve(request.result);
            request.onerror = () => reject(request.error);
        });
    ...
    async loadStoredNotifications() {
        ...
        } catch (error) { console.error('Error loading stored notifications:', error); return []; }
    }
                ...
                this.swRegistration.addEventListener('updatefound', () => {
                    ...
                    newSW.addEventListener('statechange', () => {
                        if (newSW.state === 'installed' && navigator.serviceWorker.controller) {
                            if (confirm('نسخه جدید موجود است. آیا می‌خواهید به‌روزرسانی کنید؟')) window.location.reload();
                        }
                    });
                });
                return this.swRegistration;
            } else { console.warn('Service Worker not supported'); return null; }
        } catch (error) { console.error('Service Worker registration failed:', error); return null; }
    }

    setupMessageListener() {
        if ('serviceWorker' in navigator) {
            navigator.serviceWorker.addEventListener('message', (event) => {
                if (event.data && event.data.type === 'highlight-program') this.highlightProgram(event.data.programId);
            });
        }
    }
    ...
    highlightProgram(programId) {
        ...
        cards.forEach(card => {
            ...
            if (info && info.textContent.includes(programId)) {
                ...
                setTimeout(() => { card.style.boxShadow = '';
                    card.style.transform = ''; }, 3000);
            }
        });
    }
    ...
    async checkSubscription() {
        ...
        } catch (error) { console.error('Error checking subscription:', error); return false; }
    }
    ...
    async subscribe() {
        ...
        } catch (error) { console.error('Error subscribing to push:', error); throw error; }
    }

    async sendSubscriptionToServer(subscription) {
        try {
            const response = await fetch(`${this.WORKER_URL}/api/subscribe`, {
                method: 'POST',
            ...
            if (response.ok) {
                ...
                console.log('✅ Subscription saved on server, ID:', data.id);
            } else { console.warn('Failed to send subscription to server'); }
        } catch (error) { console.warn('Server not available, subscription stored locally'); }
    }

    async sendLocalNotification(programId, title, body, icon = '/images/Kham.png') {
        const notification = { programId, title, body, timestamp: Date.now(), isRead: false, isSynced: false };
        ...
        if (Notification.permission === 'granted') {
            const options = {
                ...
                badge: icon,
                vibrate: [200, 100, 200],
                data: { programId, url: `/?page=immigration&highlight=${programId}` },
            ...
            notif.onclick = () => {
                window.focus();
                if (typeof navigateTo !== 'undefined') navigateTo('immigration');
    ...
    createNotificationCenter() {
        ...
        center.querySelector('.notif-close-btn').addEventListener('click', () => center.classList.remove('open'));
        center.querySelector('.notif-clear-btn').addEventListener('click', async () => {
            if (confirm('آیا مطمئن هستید؟')) {
                const allNotifs = await this.getNotifications();
                for (const notif of allNotifs) await this.deleteNotification(notif.id);
                this.notifications = [];
        ...
        document.addEventListener('click', (e) => {
            if (center.classList.contains('open') && !center.contains(e.target) && !e.target.closest(
                    '.notif-bell-btn')) center.classList.remove('open');
        });
        return center;
    }
    ...
    async notifyProgram(programId, programName) {
        const messages = {
            'siv': { fa: 'تغییرات جدید در برنامه SIV: اولویت‌بندی مجدد و افزایش ظرفیت',
                en: 'New SIV changes: Re-prioritization and capacity increase' },
            'p1': { fa: 'برنامه P1: مهلت ثبت‌نام تا ۳۰ جولای تمدید شد',
                en: 'P1: Registration deadline extended to July 30' },
            'p2': { fa: 'برنامه P2: نتایج جدید منتشر شد، ۵۰۰ نفر جدید پذیرفته شدند',
                en: 'P2: New results published, 500 new applicants accepted' },
            'asylum': { fa: 'تغییرات در روند پناهندگی: زمان رسیدگی کاهش یافت',
                en: 'Asylum process changes: Processing time reduced' },
            'parole': { fa: 'برنامه Parole: دستورالعمل‌های جدید برای متقاضیان',
                en: 'Parole: New guidelines for applicants' },
            'reparole': { fa: 'Re-Parole: تمدید خودکار برای متقاضیان واجد شرایط',
                en: 'Re-Parole: Auto-extension for eligible applicants' },
            'welcome': { fa: 'Welcome Corps: افزایش ظرفیت پذیرش تا ۱۰۰۰ نفر',
                en: 'Welcome Corps: Capacity increased to 1000' },
            ...
            'dv': { fa: 'DV Lottery: تاریخ ثبت‌نام از ۱۵ اکتبر آغاز می‌شود',
                en: 'DV Lottery: Registration starts October 15' },
            'i130_gc': { fa: 'I-130 با گرین کارت: افزایش تعرفه به ۵۳۵ دلار',
                en: 'I-130 with GC: Fee increased to $535' },
            'i130_us': { fa: 'I-130 با پاسپورت: تسریع در رسیدگی به درخواست‌ها',
                en: 'I-130 with US Passport: Expedited processing' }
        };
        ...
        await this.sendLocalNotification(programId, title, body);
        if (this.isSubscribed) {
            try { await this.sendPushNotification(programId, title, body); } catch (error) { console.warn(
        ...
        try {
            const response = await fetch('/api/send-notification', {
                ...
                body: JSON.stringify({
                    ...
                    payload: { title, body, programId, icon: '/images/Kham.png',
                        url: `/?page=immigration&highlight=${programId}` }
                }),
            });
            if (!response.ok) console.warn('Failed to send push notification via server');
        } catch (error) { console.warn('Push server not available, using local notification'); }
    }
